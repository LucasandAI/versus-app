...
const ClubsList: React.FC<ClubsListProps> = ({
  ...
  return <div className="p-3">
      ...
      <div className="space-y-1">
        {clubs.map(club => <div key={club.id} className="flex flex-col relative group">
            <button className={`w-full flex items-center p-2 rounded-md text-left transition-colors ${selectedClub?.id === club.id ? 'bg-primary/10 text-primary' : 'hover:bg-gray-100'}`} onClick={() => onSelectClub(club)}>
              ...
              <div className="flex-1 overflow-hidden">
                <div className="flex items-center justify-between">
                  ...
                  {unreadCounts[club.id] > 0 && <span className="ml-1 bg-red-500 text-white text-[10px] rounded-full h-4 w-4 flex items-center justify-center">
        ...
        {clubs.length === 0 && <div className="text-center py-4 text-sm text-gray-500">
