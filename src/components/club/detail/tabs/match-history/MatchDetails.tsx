...
const MatchDetails: React.FC<MatchDetailsProps> = ({
  ...
  const getLeagueImpact = () => {
    ...
    if (before.division !== after.division || before.tier !== after.tier) {
      if (after.division === before.division && after.tier! < before.tier! || after.division !== before.division) {
  ...
  return <div className="bg-white">
      <div className="p-4">
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          {match.winner && <span className={`px-3 py-1 rounded-full text-sm ${userClubWon ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
        ...
        <div className="flex h-6 rounded-full overflow-hidden mb-4">
          <div className="bg-primary text-white flex justify-center items-center text-xs px-2" style={{
          width: `${match.homeClub.totalDistance / (match.homeClub.totalDistance + match.awayClub.totalDistance) * 100}%`
        }}>
            {match.homeClub.totalDistance.toFixed(1)} km
          </div>
          <div className="bg-gray-800 text-white flex justify-center items-center text-xs px-2" style={{
          width: `${match.awayClub.totalDistance / (match.homeClub.totalDistance + match.awayClub.totalDistance) * 100}%`
        }}>
        ...
        {/* League Impact */}
        {leagueImpact && <div className="mb-4">
            <p className="text-sm">
      ...
      <div className="px-4 py-3 bg-gray-50">
        {/* Away Club Members */}
        <h4 className="font-semibold mb-3 text-base">{match.homeClub.name} Members</h4>
        <div className="space-y-2 mb-6">
          {match.homeClub.members.sort((a, b) => (b.distanceContribution || 0) - (a.distanceContribution || 0)).map(member => <div key={member.id} className="flex items-center justify-between cursor-pointer" onClick={() => onSelectUser && onSelectUser(member.id, member.name, member.avatar)}>
                <div className="flex items-center">
                  <UserAvatar name={member.name} image={member.avatar} size="sm" />
                  <span className="ml-3 hover:text-primary text-sm">{member.name}</span>
                </div>
        ...
        <div className="space-y-2">
          {match.awayClub.members.sort((a, b) => (b.distanceContribution || 0) - (a.distanceContribution || 0)).map(member => <div key={member.id} className="flex items-center justify-between cursor-pointer" onClick={() => onSelectUser && onSelectUser(member.id, member.name, member.avatar)}>
                <div className="flex items-center">
                  <UserAvatar name={member.name} image={member.avatar} size="sm" />
                  <span className="ml-3 hover:text-primary text-sm">{member.name}</span>
                </div>
